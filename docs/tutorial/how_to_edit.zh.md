...
中国大陆访问 GitHub 可能会比较慢，甚至出现时断时续的情况。

---

## Fork 仓库

!!! warning "段落等待重写：表述欠规范、需要合并到其他章节。"

你没有直接写入此 repo[^1] 的权限，不过你可以把它 fork[^2] 下来自己修改，然后在合适的时候提交一个 pr[^3] 进行合并。
[^1]: repositorie 存储库、仓库，可以理解为存放整个项目的文件夹，repo 命名为：`用户名或组织名`/`repo 名`。
[^2]: 简而言之，复制一份 repo 在你的沙盒中编辑和测试，而不会影响已有的 repo。
[^3]: pull request 如果你觉得差不多了，就可以把你的沙盒里的内容提交到原 repo（即 IPOL 共享知识库），pr 通过后，这些内容就会正式合并到知识库。
